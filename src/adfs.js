var AuthenticationContext = (function () {
   ...
   * @ignore
   */
  AuthenticationContext.prototype._renewIdToken = async function (callback, responseType) {
      ...
      try {
        var request = await fetch(this.config.tokenRefreshUri, fetchArgs);
   ...
   */
  AuthenticationContext.prototype.saveTokenFromHash = async function (requestInfo) {
      ...
      if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
          ...
          if (requestInfo.stateMatch) {
              ...
              if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.CODE)) {
                ...
                try {
                  var request = await fetch(this.config.tokenUri, fetchArgs);
                  var response = await request.json();
                  ...
                  requestInfo.parameters = Object.assign({}, requestInfo.parameters, {
                    [this.CONSTANTS.ACCESS_TOKEN]: response.access_token,
                    [this.CONSTANTS.ID_TOKEN]: response.id_token,
                    [this.CONSTANTS.EXPIRES_IN]: response.expires_in,
                    [this.CONSTANTS.REFRESH_TOKEN]: response.refresh_token,
                    [this.CONSTANTS.REFRESH_TOKEN_EXPIRES_IN]: response.refresh_token_expires_in,
                  });
                } catch (error) {
                  this.infoPii('Error :' + error + ';');
   ...
   */
  AuthenticationContext.prototype.handleWindowCallback = async function (hash, isRedirect) {
      ...
      if (this.isCallback(hash)) {
          ...
          if (this._openedWindows.length > 0 && this._openedWindows[this._openedWindows.length - 1].opener
              && this._openedWindows[this._openedWindows.length - 1].opener._adfsInstance) {
          ...
          await self.saveTokenFromHash(requestInfo);
